...
namespace ScheduleHelper
{
	...
	public sealed class ExcelWriter 
	{
        ...
        private ICellStyle _csCenterBorderAll = null;

        public ICellStyle CsCenterBorderAll
        {
            get
            {
                if (_csCenterBorderAll == null)
                {
                    _csCenterBorderAll = currentWorkbook.CreateCellStyle();
                    _csCenterBorderAll.Alignment = HorizontalAlignment.Center;
                    _csCenterBorderAll.BorderBottom = BorderStyle.Thin;
                    _csCenterBorderAll.BorderLeft = BorderStyle.Thin;
                    _csCenterBorderAll.BorderRight = BorderStyle.Thin;
                    _csCenterBorderAll.BorderTop = BorderStyle.Thin;
                }
                return _csCenterBorderAll;
            }
        }
		...
		public ExcelWriter()
		{
            ...
            si.ApplicationName = "ScheduleAssignSystem";
        ...
        public int Write(int row, int column, int cellValue)
        {
            return Write(row, column, cellValue.ToString());
        }
        ...
        public void MergeCells(int firstRow, int firstColumn, int lastRow, int lastColumn)
        {
            ...
            currentCell = this[firstRow, firstColumn];
            currentCell.CellStyle = CSAlignCenter;
        }
    ...
    public sealed class ExcelReader
    {
        ...
        public Dictionary<string, int> GetFirstSheetHeaders()
        {
            ...
            while (cell.MoveNext())
            {
                ...
                if (dic.ContainsKey(ic.StringCellValue))
                {
                    dic.Add(ic.StringCellValue + ic.ColumnIndex.ToString(), i);
                }
                else
                {
                    dic.Add(ic.StringCellValue, i);
                }
                i++;
            }
            return dic;
        ...
        private string getSheetDataBatch(Dictionary<string, int> header)
        {
            int[] fieldsIndexs = Dic2Array(header);

            return getSheetDataBatch(fieldsIndexs);
        }

        private static int[] Dic2Array(Dictionary<string, int> header)
        {
            int[] fieldsIndexs = new int[header.Count];
            int i = 0;
            foreach (KeyValuePair<string, int> kp in header)
            {
                fieldsIndexs[i] = kp.Value;
                i++;
            }
            return fieldsIndexs;
        }
        ...
        public string[] GetFirstSheetData(string strFields, string destTableName)
        {
            ...
            ActiveSheet(0);
            Dictionary<string, int> dic = GetFirstSheetHeaders();
        ...
        public DataTable FirstSheet2Table()
        {
            DataTable dt = new DataTable();
            ActiveSheet(0);
            var header = GetFirstSheetHeaders();
            foreach (KeyValuePair<string, int> kp in header)
            {
                DataColumn column = new DataColumn(kp.Key);
                if (kp.Key == "班级" || kp.Key=="周课时")
                {
            ...
            for (int r = 1; r < row_count; r++)
            {
                IRow row = _sheet.GetRow(r);
                DataRow dr=dt.NewRow();
                foreach (KeyValuePair<string, int> kp in header)
                {
                    ICell cell = row.GetCell(kp.Value);
                    if (! (cell == null))
                    {
                        if (cell.CellType == CellType.Numeric)
                        {
                            dr[kp.Value] = cell.NumericCellValue;
                        }
                        else
                        {
                            dr[kp.Value] = cell.StringCellValue;
                        }
